...
export class CaloriesForWeightGoalView extends AbstractView {
  ...
  private weeksToWeightGoalInput: HTMLInputElement | null = null
  ...
  render = (container: HTMLElement): void => {
    ...
    this.initializeInputs([
      ...
      InputFields.WEEKS_TO_WEIGHT_GOAL,
    ])
    ...
    this.weeksToWeightGoalInput = this.getElement(
      HtmlSelectors.WEEKS_TO_WEIGHT_GOAL
    ) as HTMLInputElement
  ...
  fillForm(data: Partial<User>): void {
    ...
    this.setInputValue(this.weeksToWeightGoalInput, data.weeksToWeightGoal)
  }
  ...
  updateResults(dailyCalories: number): void {
    if (!this.resultsTable) {
      return
    }

    const rows = this.resultsTable.rows
    rows[0].cells[1].textContent = dailyCalories.toString() + 'kcal/day'
  }
}
